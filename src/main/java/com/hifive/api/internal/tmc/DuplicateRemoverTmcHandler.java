...
public class DuplicateRemoverTmcHandler extends TmcHandler {
	...
	public void onMessage(EndpointContext context) throws Exception {
		...
		if (key == null) { // 没有提供key选择器，则走正常流程
			super.handleMessage(msg);
		} else { // 走去重流程
			boolean isOk = put(key, msg);
			if (!isOk) { // 去重成功，直接confirm
				tmcClient.getThreadPool().submit(new Runnable() {
					public void run() {
						try {
							...
							log.warn(String.format("confirm message fail: %s", rawMsg), e);
						}
					}
				});
			}
	...
	private class MsgKeySelector implements KeySelector {
		public String selectKey(Message message) {
			...
			String key = null;
			if (topic.startsWith("Hifive_trade")||topic.equals("Hifive_datapush_SynTrade")) {
				key = "trade_" + this.getId(message, "tid");
			} else if (topic.startsWith("Hifive_item")) {
				key = "item_" + this.getId(message, "num_iid");
			} else if (topic.startsWith("Hifive_refund")) {
		...
		public void run() {
			...
			while (hasConsumeNum.get() < leftNum) {
				...
				try {
					...
					handleMessage(msgMap.remove(key));
